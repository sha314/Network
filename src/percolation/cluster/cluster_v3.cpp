...
void Cluster_v3::add_nodes(const std::vector<Node> &nds) {
...
//void Cluster_v3::add_link_indices(const std::vector<Link> &nds) {
...
//void Cluster_v3::add_link_index(const Link &lnk) {
//    _links_indices.push_back(lnk.getIndex());
//}

void Cluster_v3::add_node(const Node &nds) {
    _nodes.push_back(nds.get_id());
}

void Cluster_v3::viewNodes() {
    cout << "{";
...
//void Cluster_v3::viewLinkIndices() {
//    cout << "{";
...
void Cluster_v3::insert(const Cluster_v3 &clstr) {
...
double Cluster_v3::size_in_MB() {

    double sz = sizeof(int) + sizeof(uint) * _nodes.size() + sizeof(size_t);
